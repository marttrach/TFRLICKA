...
- **TRA 影像辨識** — 從票券、訂票結果與時刻表截圖擷取車次、站名、日期及時間
- **一般圖片 OCR** — 辨識繁中與英文，結果可修正並一鍵複製
...
儀表板的「台鐵資訊擷取」模式會整理車次、站名、日期、時間與可能路線，並提供摘要複製。
結果不會自動寫入訂票條件，使用者必須對照原圖核對。

`reference/thsr-legacy/` 保存的是未接線的 THSR 歷史模型與流程，不會進入 Python 套件或
Docker 映像，也不是 TRA 模型。TRA 影像功能不處理 CAPTCHA 或 reCAPTCHA。
